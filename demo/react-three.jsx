...
export function Canvas({ children, ...props }) {
  ...
  if (canvas) {
    ...
    if (props.gl) applyProps(store.gl, props.gl)

    if (!store.camera) {
      store.camera = new THREE.PerspectiveCamera(75)
      store.camera.position.z = 5
    }
    if (props.camera) applyProps(store.camera, props.camera)

    content = reconciler.createRoot(<context.Provider value={store}>{children}</context.Provider>, store)
  }
